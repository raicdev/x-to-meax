import { NitterRssClient, parseNitterRss } from "./nitterRssClient.js";
...
function buildClient() {
  ...
  const rssUrl = positionalUrl || process.env.NITTER_RSS_URL;

  if (!rssUrl && !username) {
    throw new Error("Set NITTER_RSS_URL or X_USERNAME before running debug:rss.");
  }
...
async function main() {
  ...
  const directPosts = parseNitterRss(body);
